...
export const useAdmin = (currentUser, seasonName, users, roles = []) => {
    ...
    const recalculateUserPoints = async (username, userDocId, currentSeason, currentRoles = roles) => {
        ...
        // 🔥 使用 taskId 作為 key 確保同一個任務不重複計分
        const uniqueSubsByTask = new Map();
        results.forEach(snapshot => {
            snapshot.forEach(doc => {
                const data = doc.data();
                const taskId = data.taskId;
                // 如果已經有該任務的通過紀錄，則不再加入 (或者可以比對 timestamp 留最新的)
                if (!uniqueSubsByTask.has(taskId)) {
                    uniqueSubsByTask.set(taskId, data);
                }
            });
        });
